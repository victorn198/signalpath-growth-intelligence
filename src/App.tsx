...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main id="top">
      ...
      <section className="filterbar"><div className="filter-heading"><span>{t.filters}</span><small>{data.meta.rows.toLocaleString()} rows</small></div><div className="period-control"><span>{lang==='pt'?'Período':'Period'}</span><div className="period-options">{periodOptions.map(([value,label])=><button key={value} className={filters.period===value?'active':''} disabled={queryState==='loading'} onClick={()=>{setDrillPath([]);setFilters(v=>({...v,period:value}))}}>{label}</button>)}</div></div><div className="comparison-contract"><span>{lang==='pt'?'Comparar com':'Compare with'}</span><b>{filters.period==='all'?(lang==='pt'?'Sem baseline':'No baseline'):(lang==='pt'?'Período anterior equivalente':'Previous equivalent period')}</b></div>{select(t.channel, 'channel', data.filters.channels)}{select(t.device, 'device', data.filters.devices)}{select(t.country, 'country', data.filters.countries)}<button className="reset" onClick={() => {setDrillPath([]);setFilters({ channel: '', device: '', country: '', period:'30' })}}><RotateCcw size={15} />{t.reset}</button></section>
      ...
      <p className="scope-caption"><b>{mart.context.currentStart} — {mart.context.currentEnd}</b> · {comparisonAvailable ? `${lang==='pt'?'comparado com':'compared with'} ${mart.context.previousStart} — ${mart.context.previousEnd}` : (lang==='pt'?'histórico completo, sem baseline':'complete history, no baseline')}</p>
