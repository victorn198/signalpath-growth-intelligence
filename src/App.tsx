...
export default function App() {
  ...
  const findingByPage:Record<string,{en:string;pt:string}>={
    ...
    funnel:{en:`Scoped conversion is ${((liveValues.conversion??0)*100).toFixed(1)}%. The full-source event-reach gap is largest between ${leak?`${leak.from} and ${leak.to}`:'unavailable stages'}. Independent audiences do not measure sequential abandonment.`,pt:`A conversão do recorte é ${((liveValues.conversion??0)*100).toFixed(1)}%. A maior diferença de alcance na fonte completa está entre ${leak?`${leak.from} e ${leak.to}`:'etapas indisponíveis'}. Audiências independentes não medem abandono sequencial.`},
