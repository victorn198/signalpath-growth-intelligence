...
echarts.use([EBarChart, ELineChart, EScatterChart, GridComponent, MarkPointComponent, TooltipComponent, LegacyGridContainLabel, CanvasRenderer])
...
export function BarChart({ data, lang, onSelect }: { data: BreakdownRow[]; lang: Lang; onSelect?:(name:string)=>void }) {
  ...
  const previousMarkers=seriesData.flatMap((row,index)=>row.hasPrevious?[{coord:[row.previous,index],symbol:visual==='bar'?'rect':'circle',symbolSize:visual==='bar'?[3,18]:14}]:[])
  const marker={silent:true,label:{show:false},itemStyle:{color:'#73839a'},data:previousMarkers}
  const currentSeries=visual==='bar'?{type:'bar',data:seriesData,barWidth:10,itemStyle:{color:'#1f6feb',borderRadius:[0,4,4,0]},emphasis:{disabled:true},markPoint:marker,z:2,label:{show:true,position:'right',formatter:label,rich:{value:{color:'#415269'},divider:{color:'#aab4c3'},up:{color:'#07855b',fontWeight:700},down:{color:'#d9364f',fontWeight:700},new:{color:'#1f6feb',fontWeight:700},neutral:{color:'#7c8798'}}}}:{type:'scatter',data:seriesData,symbolSize:11,itemStyle:{color:'#625bf6'},emphasis:{disabled:true},markPoint:marker,z:2,label:{show:true,position:'right',formatter:label,rich:{value:{color:'#415269'},divider:{color:'#aab4c3'},up:{color:'#07855b',fontWeight:700},down:{color:'#d9364f',fontWeight:700},new:{color:'#1f6feb',fontWeight:700},neutral:{color:'#7c8798'}}}}
  const option = { animationDuration: 350, grid: { left: 12, right: 120, top: 12, bottom: 12, containLabel: true }, tooltip: { trigger: 'item',formatter:(p:{name:string}&LabelPoint)=>`${p.name}<br/>${format(p.data.value)} | ${comparison(p)}` }, xAxis: { type: 'value', max:measure==='share'?100:undefined,splitNumber:4,splitLine:{lineStyle:{color:'#edf1f7'}},axisLabel:{color:'#697386',formatter:format}},yAxis:{type:'category',data:sorted.map(d=>d.name),axisLine:{show:false},axisTick:{show:false},axisLabel:{color:'#27364b',width:140,overflow:'truncate'}},series:[currentSeries] }
  const m=lang==='pt'?{value:'Valor',share:'Participação',bar:'Barras',dot:'Pontos'}:{value:'Value',share:'Share',bar:'Bars',dot:'Dots'}
  return <><div className="chart-modes split"><div>{(['value','share'] as const).map(item=><button className={measure===item?'active':''} onClick={()=>setMeasure(item)} key={item}>{m[item]}</button>)}</div><small className="comparison-key">{lang==='pt'?'Barra: atual · marca: anterior':'Bar: current · marker: prior'}</small><div>{(['bar','dot'] as const).map(item=><button className={visual===item?'active':''} onClick={()=>setVisual(item)} key={item}>{m[item]}</button>)}</div></div><div className="chart-viewport" data-breakdown-signature={sorted.map(row=>`${row.name}:${row.value}:${row.previous??'none'}`).join('|')}><ReactEChartsCore echarts={echarts} option={option} onEvents={onSelect?{click:(params:{name:string})=>onSelect(params.name)}:undefined} style={{height:300,width:'100%'}}/></div></>
}
